package edu.teamrocket.test_dni;

import edu.teamrocket.dni.Tabla_asignacion;
...
public class Test_tabla_asignacion {

    private static Tabla_asignacion tabla;
    private static final char[] letrasNoPermitidas = {'I', 'Ñ', 'O', 'U'};

    @BeforeClass
    public static void crearTabla() {
        tabla = new Tabla_asignacion();
    }

    @Test
    public void getLetraTablaAsignacion() {
        assertEquals('T', tabla.get_letra(0));
        assertEquals('E' , tabla.get_letra(22));
    }
    ...
    public void letraNoPermitida() {
        ...
        for (char letra : letrasNoPermitidas) {
            assertFalse(tabla.is_letra_permitida(letra));
        }
    }

    private static boolean testCalcularLetra(List<String> casosTest, Tabla_asignacion tabla){
        ...
        for (String dni : casosTest) {
            ...
            if (tabla.calcular_letra( parteNumericaDni ) == letraDni) {
                continue;
            } else {
                return false;
            }
